...
impl<Req: PipelineIO, Resp: PipelineIO> Ingress<Req, Resp> {
    ...
    /// frame, with the same metrics + cancellation accounting both unary
    /// and bidirectional `PushWorkHandler` impls need. Consumes the
    /// publisher — pump-and-done.
        ...
        while let Some(resp) = stream.next().await {
            ...
            if publisher.send(resp_bytes.into()).await.is_err() {
                ...
                if context.is_stopped() {
                    // Two interleavings produce this branch:
                    ...
                    //      → context.is_stopped (this)
                    // Case 1 happens on a normal client-side disconnect after the
                    // full response was already received; expected, not an error.
                ...
                // the engine is healthy and shouldn't reset the canary timer.
                notifier.notify_one();
            }
        }
