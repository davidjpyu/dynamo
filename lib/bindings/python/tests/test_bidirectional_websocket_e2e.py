...
The wire shapes emitted by the Python handler mirror the Rust
`EchoBidirectionalEngine` and the Rust-side `WireEchoEngine` covered by
`lib/llm/tests/http_websocket_python_bridge.rs`. The frontend's typed
reader requires fully-populated `RealtimeServerEvent` payloads —
in particular `RealtimeResponse` needs `id`, `max_output_tokens`,
